...
> = {
  DRAFT: {
    color: "text-[hsl(var(--vp-muted))]",
    bg: "bg-[hsl(var(--vp-bg-soft))]",
    icon: Clock,
  ...
  PARTIAL: {
    ...
    icon: Package,
    labelKey: "stats.partial",
  },
  RECEIVED: {
    color: "text-emerald-600 dark:text-emerald-400",
    bg: "bg-emerald-50 dark:bg-emerald-900/20",
    icon: CheckCircle,
    labelKey: "stats.received",
  },
  CANCELLED: {
...
export default function PurchaseOrdersPage() {
  ...
  const loadSuggestions = useCallback(async () => {
    ...
    try {
      const [suggestionsRes, linksRes] = await Promise.all([
        apiFetch(
          `/api/purchase-orders/suggestions?supplierId=${selectedSupplierId}&coverageDays=${coverageDays}`,
        ),
  ...
  const addSuggestionToOrder = (s: Suggestion) => {
    ...
    const reasonString =
      s.reasonParts
        ?.map((part) =>
          t(part.key, "purchaseOrders", {
            ...part.params,
            unit: t("create.unitAbr", "purchaseOrders"),
          }),
        )
        .join(" | ") || s.reason;
  ...
  const addManualProduct = async () => {
    ...
    const isLinked = supplierLinks.some(
      (l) =>
        String(l.productId._id) === manualProductId ||
        String(l.productId) === manualProductId,
    );
    ...
    if (!isLinked && shouldAutoLink) {
      try {
        ...
        const linksRes = await apiFetch(
          `/api/product-suppliers?supplierId=${selectedSupplierId}`,
        );
  ...
  const createOrder = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const errMsg =
          err.error && err.error.startsWith("errors.")
  ...
  const receiveOrder = async () => {
    ...
    const result = await updateOrderStatus(selectedOrder._id, "receive", {
      receivedItems,
    });
    if (result.success) {
      setViewMode("detail");
  ...
  const filteredOrders = orders.filter((o) => {
    ...
    if (searchQuery) {
      ...
      const supplierName =
        typeof o.supplierId === "object" ? o.supplierId.name : "";
      return (
        ...
        <main className="max-w-4xl mx-auto px-6 py-20 text-center">
          <div className="vp-card p-12 border-dashed border-2 border-[hsl(var(--vp-primary)/0.3)] bg-[hsl(var(--vp-primary)/0.05)]">
            ...
            <p className="text-lg opacity-70 mb-8">
              Las órdenes de compra inteligentes y la gestión avanzada de stock
              están disponibles únicamente en planes Pro.
            </p>
    ...
    <div className="vp-page">
      ...
      <main className="vp-page-inner">
        ...
        {viewMode === "list" && (
          <div className="space-y-6 vp-fade-in">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
              {[
                {
                  label: t("stats.total", "purchaseOrders"),
                  value: stats.total,
                  icon: ClipboardCheck,
                  color: "var(--vp-primary)",
                },
                {
                  label: t("stats.draft", "purchaseOrders"),
                  value: stats.draft,
                  icon: Clock,
                  color: "var(--vp-muted)",
                },
                {
                  label: t("stats.sent", "purchaseOrders"),
                  value: stats.sent,
                  icon: Send,
                  color: "201 96% 56%",
                },
                {
                  label: t("stats.partial", "purchaseOrders"),
                  value: stats.partial,
                  icon: Package,
                  color: "var(--vp-warning)",
                },
                {
                  label: t("stats.received", "purchaseOrders"),
                ...
                <div
                  key={stat.label}
                  className="vp-card p-4 flex items-center gap-3"
                >
                  <div
                    className="p-2.5 rounded-xl"
                    style={{ background: `hsl(${stat.color} / 0.12)` }}
                  >
                    <stat.icon
                      className="w-5 h-5"
            ...
            <div className="vp-card p-4 flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  placeholder={t("filters.searchPlaceholder", "purchaseOrders")}
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                />
              </div>
              <select
                className="vp-input w-full sm:w-48"
                value={statusFilter}
                onChange={(e) => setStatusFilter(e.target.value)}
              >
                <option value="">
                  {t("filters.allStatuses", "purchaseOrders")}
                </option>
                {Object.entries(STATUS_CONFIG).map(([k, v]) => (
                  <option key={k} value={k}>
                    {t(v.labelKey, "purchaseOrders")}
                  </option>
                ))}
              </select>
            </div>
              ...
              {filteredOrders.length > 0 ? (
                <div className="overflow-x-auto">
                  <table className="vp-table">
                    <thead>
                      <tr>
                        <th>{t("table.number", "purchaseOrders")}</th>
                        <th>{t("table.supplier", "purchaseOrders")}</th>
                        <th>{t("table.date", "purchaseOrders")}</th>
                        <th>{t("table.status", "purchaseOrders")}</th>
                        <th>{t("table.items", "purchaseOrders")}</th>
                        <th className="text-right">
                          {t("table.totalEst", "purchaseOrders")}
                        </th>
                        <th className="text-right">
                          {t("table.actions", "purchaseOrders")}
                        </th>
                      </tr>
                    </thead>
                    <tbody>
                      {filteredOrders.map((order) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td>
                              <span
                                className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${sc.color} ${sc.bg}`}
                              >
                                <StatusIcon className="w-3.5 h-3.5" />{" "}
                                {t(sc.labelKey, "purchaseOrders")}
                              </span>
                            </td>
                            ...
                            >
                              {order.status === "DRAFT" && (
                                <button
                                  className="vp-button vp-button-sm vp-button-primary vp-press gap-1.5"
                                  disabled={actionLoading}
                                  onClick={() =>
                                    updateOrderStatus(order._id, "send")
                                  }
                                >
                                  {actionLoading ? (
                              ...
                              {(order.status === "SENT" ||
                                ...
                                <button
                                  className="vp-button vp-button-sm vp-press gap-1.5"
                                  disabled={actionLoading}
                                  style={{
                                    borderColor: "hsl(var(--vp-success))",
                                    color: "hsl(var(--vp-success))",
                                  }}
                                  onClick={() => {
                                    setSelectedOrder(order);
                                    setReceivedItems(
                                ...
                                >
                                  ...
                                  {t("table.receive", "purchaseOrders")}
                                </button>
                              )}
                            </td>
                          </tr>
                ...
                <div className="vp-empty-state m-6">
                  ...
                  <p className="font-semibold text-[hsl(var(--vp-text))]">
                    {t("table.noOrders", "purchaseOrders")}
                  </p>
                  <p className="text-sm mt-1">
                    {t("table.noOrdersSubtitle", "purchaseOrders")}
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 space-y-5">
                <div className="vp-card p-5 space-y-4">
                  <h2 className="font-semibold text-[hsl(var(--vp-text))]">
                    {t("create.generalInfo", "purchaseOrders")}
                  </h2>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="vp-label">
                        {t("create.supplier", "purchaseOrders")}
                      </label>
                      <select
                        className="vp-input"
                      ...
                      >
                        <option value="">
                          {t("create.selectSupplier", "purchaseOrders")}
                        </option>
                        {suppliers.map((s) => (
                    ...
                    <div>
                      <label className="vp-label">
                        {t("create.deliveryDate", "purchaseOrders")}
                      </label>
                      <input
                        type="date"
                        className="vp-input"
                    ...
                    <div>
                      <label className="vp-label">
                        {t("create.warehouse", "purchaseOrders")}
                      </label>
                      <input
                        className="vp-input"
                        placeholder={t(
                          "create.warehousePlaceholder",
                          "purchaseOrders",
                        )}
                ...
                {selectedSupplierId && (
                  <div
                    className="vp-card p-5 border-[hsl(var(--vp-primary)/0.3)]"
                    style={{
                      background:
                        "linear-gradient(135deg, hsl(var(--vp-bg-card)), hsl(var(--vp-primary) / 0.04))",
                    }}
                  >
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <h3 className="font-semibold text-[hsl(var(--vp-text))]">
                          {t("create.aiSuggestions", "purchaseOrders")}
                        </h3>
                        <span className="vp-pill text-xs">
                          {t("create.aiPill", "purchaseOrders")}
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
                        <label className="text-xs text-[hsl(var(--vp-muted))]">
                          {t("create.coverage", "purchaseOrders")}
                        </label>
                        <select
                          className="vp-input text-xs !min-h-[32px] !py-1 w-20"
                          value={coverageDays}
                          onChange={(e) =>
                            setCoverageDays(Number(e.target.value))
                          }
                        >
                          <option value={7}>
                            {t("create.days", "purchaseOrders", { count: 7 })}
                          </option>
                          <option value={14}>
                            {t("create.days", "purchaseOrders", { count: 14 })}
                          </option>
                          <option value={21}>
                            {t("create.days", "purchaseOrders", { count: 21 })}
                          </option>
                          <option value={30}>
                            {t("create.days", "purchaseOrders", { count: 30 })}
                          </option>
                        </select>
                        <button
                          className="vp-button vp-button-primary vp-button-sm vp-press"
                          onClick={loadSuggestions}
                          disabled={suggestionsLoading}
                        >
                          <Sparkles className="w-4 h-4" />
                          {suggestionsLoading
                            ? t("create.analyzing", "purchaseOrders")
                            : t("create.generate", "purchaseOrders")}
                        </button>
                      </div>
                    </div>
                    ...
                    {suggestionsLoading ? (
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between items-center">
                          <p className="text-xs text-[hsl(var(--vp-muted))]">
                            {t("create.suggestedProducts", "purchaseOrders", {
                              count: suggestions.length,
                            })}
                          </p>
                          <button
                            className="text-xs font-semibold text-[hsl(var(--vp-primary))] hover:underline"
                            ...
                            return (
                              ...
                              >
                                <div className="flex-1 min-w-0">
                                  <div className="flex items-center gap-2">
                                    <span
                                      className={`px-2 py-0.5 rounded-full text-xs font-bold ${pc.color} ${pc.bg}`}
                                    >
                                ...
                                <div className="flex items-center gap-3 ml-3 shrink-0">
                                  <div className="text-right">
                                    <p className="text-sm font-semibold">
                                      {s.suggestedQuantity}{" "}
                                      {t("create.unitAbr", "purchaseOrders")}
                                    </p>
                                    <p className="text-xs text-[hsl(var(--vp-muted))]">
                                  ...
                                  <button
                                    className="vp-button vp-button-sm vp-button-primary vp-press"
                                    onClick={() => addSuggestionToOrder(s)}
                      ...
                      showSuggestions && (
                        <div className="p-6 text-center bg-[hsl(var(--vp-bg-soft))] rounded-xl border border-[hsl(var(--vp-border-soft))]">
                          <p className="text-sm text-[hsl(var(--vp-muted))] mb-1">
                            {t("create.noSuggestions", "purchaseOrders") ||
                              "No suggestions found for this supplier."}
                          </p>
                          <p className="text-xs text-[hsl(var(--vp-muted))] opacity-75">
                            {t(
                              "create.noSuggestionsSubtitle",
                              "purchaseOrders",
                            ) ||
                              "Linking products to this supplier or recording more sales will help the AI."}
                          </p>
                        </div>
                      )
                    )}
                ...
                <div className="vp-card p-5">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button
                      className="vp-button vp-button-sm vp-press"
                      onClick={() => setShowAddProduct(true)}
                    >
                      <Plus className="w-4 h-4" />{" "}
                      {t("create.manualAdd", "purchaseOrders")}
                    </button>
                  </div>
                  ...
                  {showAddProduct && (
                    <div className="p-4 mb-4 rounded-xl border border-[hsl(var(--vp-border))] bg-[hsl(var(--vp-bg-soft))]">
                      <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                        <select
                          className="vp-input sm:col-span-2"
                        ...
                        >
                          <option value="">
                            {t("create.selectProduct", "purchaseOrders")}
                          </option>
                          {products.map((p) => (
                        ...
                        <input
                          type="number"
                          className="vp-input"
                          placeholder={t("create.quantity", "purchaseOrders")}
                          min={1}
                        ...
                        <input
                          type="number"
                          className="vp-input"
                          placeholder={t("create.cost", "purchaseOrders")}
                          step="0.01"
                          min={0}
                          value={manualCost}
                          onChange={(e) =>
                            setManualCost(Number(e.target.value))
                          }
                        />
                      </div>
                          ...
                          <div className="mt-3 p-3 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/50">
                            ...
                            <label className="flex items-center gap-2 cursor-pointer group">
                              <input
                                ...
                                checked={shouldAutoLink}
                                onChange={(e) =>
                                  setShouldAutoLink(e.target.checked)
                                }
                              />
                      ...
                      <div className="flex justify-end gap-2 mt-3">
                        <button
                          className="vp-button vp-button-sm"
                      ...
                      <table className="vp-table">
                        <thead>
                          <tr>
                            ...
                            <th>{t("table.code", "purchaseOrders")}</th>
                            <th className="text-right">
                              {t("create.quantity", "purchaseOrders")}
                            </th>
                            <th className="text-right">
                              {t("create.cost", "purchaseOrders")}
                            </th>
                            <th className="text-right">
                              {t("detail.subtotal", "purchaseOrders")}
                            </th>
                            <th></th>
                          </tr>
                        </thead>
                          ...
                          {orderItems.map((item, idx) => (
                            <tr key={item.productId}>
                              <td>
                                ...
                                {item.reason && (
                                  <p className="text-xs text-[hsl(var(--vp-muted))] mt-0.5 truncate max-w-[200px]">
                                    {item.reason}
                                  </p>
                                )}
                              </td>
                              ...
                              <td className="text-right">
                                <input
                                  type="number"
                                  className="vp-input !min-h-[32px] !py-1 w-20 text-right"
                                  value={item.quantity}
                                  min={1}
                                  onChange={(e) => {
                                    const newItems = [...orderItems];
                              ...
                              <td className="text-right">
                                <input
                                  type="number"
                                  className="vp-input !min-h-[32px] !py-1 w-24 text-right"
                                  value={item.cost}
                                  step="0.01"
                                  min={0}
                                  onChange={(e) => {
              ...
              <div className="space-y-4">
                <div className="vp-card p-5 sticky top-32">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between">
                      <span className="font-semibold text-[hsl(var(--vp-text))]">
                        {t("create.totalEstimated", "purchaseOrders")}
                      </span>
                  ...
                  <button
                    className="vp-button vp-button-primary vp-press w-full py-3 text-lg flex items-center justify-center gap-3 mt-5"
                    onClick={createOrder}
                    disabled={
                      actionLoading ||
                      orderItems.length === 0 ||
        ...
        {viewMode === "detail" &&
          ...
          (() => {
            const sc =
              STATUS_CONFIG[selectedOrder.status] || STATUS_CONFIG.DRAFT;
            const StatusIcon = sc.icon;
            const supplier =
              typeof selectedOrder.supplierId === "object"
              ...
              <div className="space-y-6 vp-fade-in">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex gap-2">
                    {selectedOrder.status === "DRAFT" && (
                      <>
                        <button
                          className="vp-button vp-button-primary vp-button-sm vp-press gap-1.5"
                          disabled={actionLoading}
                          onClick={() =>
                            updateOrderStatus(selectedOrder._id, "send")
                          }
                        >
                          {actionLoading ? (
                            <div className="animate-spin rounded-full h-3 w-3 border-b-2 border-white" />
                        ...
                        <button
                          className="vp-button vp-button-sm vp-press text-rose-600 border-rose-300 gap-1.5"
                          disabled={actionLoading}
                          onClick={() => {
                            if (
                              confirm(t("messages.confirmAction", "common"))
                            ) {
                        ...
                        >
                          ...
                          {t("stats.cancelled", "purchaseOrders")}
                        </button>
                      </>
                    )}
                    {(selectedOrder.status === "SENT" ||
                      ...
                      >
                        ...
                        {t("detail.receiveAction", "purchaseOrders")}
                      </button>
                    )}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div className="vp-card p-4">
                    ...
                    <p className="text-lg font-bold text-[hsl(var(--vp-primary))]">
                      ${selectedOrder.estimatedTotal.toFixed(2)}
                    </p>
                  </div>
                  <div className="vp-card p-4">
                    <p className="text-xs text-[hsl(var(--vp-muted))]">
                      {t("detail.finalTotal", "purchaseOrders")}
                    </p>
                    <p className="text-lg font-bold text-[hsl(var(--vp-text))]">
                ...
                <div className="vp-card overflow-hidden">
                  <div className="overflow-x-auto">
                    <table className="vp-table">
                      <thead>
                        <tr>
                          <th>{t("create.selectProduct", "purchaseOrders")}</th>
                          <th>{t("table.code", "purchaseOrders")}</th>
                          <th className="text-right">
                            {t("detail.requested", "purchaseOrders")}
                          </th>
                          <th className="text-right">
                            {t("detail.received", "purchaseOrders")}
                          </th>
                          <th className="text-right">
                            {t("detail.costEst", "purchaseOrders")}
                          </th>
                          <th className="text-right">
                            {t("detail.costFinal", "purchaseOrders")}
                          </th>
                          <th className="text-right">
                            {t("detail.subtotal", "purchaseOrders")}
                          </th>
                      ...
                      <tbody>
                        {selectedOrder.items.map((item) => {
                          ...
                          const partial =
                            item.receivedQuantity > 0 && !received;
                          return (
                            <tr key={item.productId}>
                              <td>
                                <span className="font-medium">
                ...
                {selectedOrder.notes && (
                  <div className="vp-card p-4">
                    <p className="text-xs text-[hsl(var(--vp-muted))] mb-1">
                      {t("create.notes", "purchaseOrders")}
                    </p>
        ...
        {viewMode === "receive" && selectedOrder && (
          <div className="space-y-6 vp-fade-in">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-[hsl(var(--vp-text))]">
                  ...
                  {t("reception.title", "purchaseOrders")}:{" "}
                  {selectedOrder.orderNumber}
                </h1>
                <p className="text-sm text-[hsl(var(--vp-muted))]">
                ...
                <table className="vp-table">
                  <thead>
                    <tr>
                      ...
                      <th className="text-right">
                        {t("reception.alreadyReceived", "purchaseOrders")}
                      </th>
                      <th className="text-right">
                        {t("reception.receiveNow", "purchaseOrders")}
                      </th>
                      <th className="text-right">
                        {t("detail.costFinal", "purchaseOrders")}
                      </th>
                    ...
                    {selectedOrder.items.map((item, idx) => {
                      ...
                      return (
                        <tr key={item.productId}>
                          ...
                          <td className="text-right text-[hsl(var(--vp-muted))]">
                            {item.receivedQuantity}
                          </td>
                          <td className="text-right">
                            <input
            ...
            <div className="flex justify-end gap-3">
              <button
                className="vp-button vp-press"
                onClick={() => setViewMode("detail")}
              >
                {t("cancel", "common")}
              </button>
              <button
                className="vp-button vp-button-primary vp-press flex items-center gap-2"
