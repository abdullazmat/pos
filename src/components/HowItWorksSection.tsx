  ...
  return (
    ...
    >
      <div className="absolute inset-0 -z-10">
        ...
        <div
          className="absolute -top-24 left-[-5%] h-72 w-72 rounded-full bg-[hsl(var(--vp-primary))]/12 blur-3xl vp-orb vp-orb-fast"
          style={{ "--vp-orb-delay": "-6s" } as CSSProperties}
        />
        <div
          className="absolute bottom-[-30%] right-[-10%] h-96 w-96 rounded-full bg-[hsl(var(--vp-primary))]/8 blur-[140px] vp-orb vp-orb-slow"
          style={{ "--vp-orb-delay": "-12s" } as CSSProperties}
        />
      </div>
        ...
        <div className="rounded-3xl border border-[hsl(var(--vp-border))] bg-[hsl(var(--vp-bg))]/70 px-5 sm:px-6 py-12 sm:py-14 shadow-[0_30px_80px_-60px_rgba(15,23,42,0.8)] backdrop-blur">
          ...
          <div className="grid gap-5 md:grid-cols-3 sm:gap-6">
            {content?.steps?.map((step, index) => (
              <div
                key={step.title}
                className="relative group vp-reveal"
                style={
                  { "--vp-reveal-delay": `${index * 120}ms` } as CSSProperties
                }
              >
